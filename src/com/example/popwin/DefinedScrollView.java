...
//		final int destScreen = (getScrollY() + screenHeight / 2) / screenHeight;
	...
	*/
	public void snapToScreen(int whichScreen) {
		...
		System.out.println(whichScreen+"-----snapToScreen");
		// get the valid layout page
		System.out.println(MainActivity.getarrDownMap()+" "+(MainActivity.getarrDownMap()-1)/4);
		whichScreen = Math.min(whichScreen, (MainActivity.getarrDownMap()-1)/4);
...
//				ActContentByDefinedView.pageCount=whichScreen+1;
//				ActContentByDefinedView.setupView(MainActivity.ds);
...
//		whichScreen = Math.max(0, Math.min(whichScreen, getChildCount() - 1));
		whichScreen = Math.max(0, Math.min(whichScreen, (MainActivity.arrDownMap.size()+3)/4));
...
//					snapToScreen(Math.min(getCurScreen() - 1,(MainActivity.arrDownMap.size()-1)/4));//I changed here
...
//				else if (velocityY < -SNAP_VELOCITY && getCurScreen() < getChildCount() - 1) { 
					...
					snapToScreen(getCurScreen() + 1);
//					snapToScreen(Math.min(getCurScreen() + 1,(MainActivity.getarrDownMap()-1)/4));
					//++Configure.curentPage; 
					//pageListener.page(Configure.curentPage);
				} else {
					snapToDestination();
